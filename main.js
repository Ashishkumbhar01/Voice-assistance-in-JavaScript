...
let timing = [`Time is now ${times}`, `Today is ${day}`, `Sir today is ${day} now`];


const SpeechRecognition = window.SpeechRecognition || window.mozSpeechRecognition || window.webkitSpeechRecognition;
...
function chatbotvoice(message) {
   ...
   speech.text = "I am don't understand";
   ...
   if (message.includes('what is time now' || 'which day' || 'what is day now')) {
      let finalresult = timing[Math.floor(Math.random() * closing.length)];
      speech.text = finalresult;
   }
